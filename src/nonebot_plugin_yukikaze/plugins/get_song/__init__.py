...
async def _(message: Message = CommandArg()):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(get_song_audio + str(song_id) + get_song_audio2) as response:
            # 获取歌曲链接
        ...
        async with session.get(get_song_info + str(song_id)) as response:
            ...
            # 发送歌曲
    sand_msg = onebotv11MessageSegment.music_custom(url=song_url,audio=song_audio,title=song_name,img_url=song_img,content=song_author)
